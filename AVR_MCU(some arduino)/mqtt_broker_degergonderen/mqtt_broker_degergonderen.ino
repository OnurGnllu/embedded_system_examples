...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (message == "led_on") {
    ...
    delay(500);
   client.publish("onurgnllu/f/situation", "evdeki led acildi");
    Serial.print("\nled yandi");
  }   // LED on
  if (message == "led_off") {
    digitalWrite(BUILTIN_LED, HIGH);
    delay(500);
   client.publish("onurgnllu/f/situation", "evdeki led kapandı");
  } // LED off
  Serial.println();
  Serial.println("-----------------------");
...
// save the last time you blinked the LED
 last_run = current_time;
  ...
  String ldrValue = String(sensorValue); //sensorvalue yu tamamen string yapmak için kullanılan parametre..
